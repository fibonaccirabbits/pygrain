...
class Cg():
  ...
  def gmx_minim_eq(self):
    '''executes the following genion, minim, nvt,
    and npt '''
    protfiles  = self.findfiles('prot.pdb') 
    for protfile in protfiles[:]:
      protpath, protnamex, protname = self.path2names(protfile)
      ...
      while nptfound == False:
        ...
        systemfile = protpath + '/system.gro'
        os.chdir(protpath)
        ionout = self.do_genion(ionsmdp, systemfile)
        if ionout[0] not in os.listdir(protpath):
          print 'this step failed:\n %s' % ionout[1]
          print 'for this system:\n %s' % protpath
          sys.exit('exiting gmx mini eq')
        minimout = self.do_minim(minimmdp, systemfile)
        if minimout[0] not in os.listdir(protpath):
          print 'this step failed:\n %s' % minimnout[1]
          print 'for this system:\n %s' % protpath
          sys.exit('exiting gmx mini eq')
        nvtout = self.do_nvt(nvtmdp, systemfile)
        if nvtout[0] not in os.listdir(protpath):
          print 'this step failed:\n %s' % nvtout[1]
          print 'for this system:\n %s' % protpath
          sys.exit('exiting gmx mini eq')
        nptout = self.do_npt(nptmdp, systemfile)
        if nptout[0] not in os.listdir(protpath):
          print 'this step failed:\n %s' % nptout[1]
          print 'for this system:\n %s' % protpath
          sys.exit('exiting gmx mini eq')
        dircontents2 = os.listdir(protpath)
